...
export default function TeacherAttendancePage() {
  ...
  async function loadData() {
    ...
    try {
      const savedTeacherId = Number(localStorage.getItem("attendance_teacher_id") || "0"); if (!savedTeacherId) { throw new Error("Teacher session not found."); } let assignedStudentIds: number[] = []; if (savedTeacherId !== 1) { const { data: assignedData, error: assignedError } = await supabase.rpc("get_teacher_assigned_student_ids", { p_teacher_id: savedTeacherId }); if (assignedError) throw new Error(assignedError.message); assignedStudentIds = (assignedData || []).map((item: { student_id: number }) => Number(item.student_id)); } let studentQuery = supabase.from("students").select("id, student_name, student_username").order("id", { ascending: true }); if (savedTeacherId !== 1) { if (assignedStudentIds.length === 0) { setStudents([]); setAttendance([]); setLoading(false); return; } studentQuery = studentQuery.in("id", assignedStudentIds); } const { data: studentData, error: studentError } = await studentQuery;

      if (studentError) {
        throw new Error(studentError.message);
      }

      let attendanceQuery = supabase.from("attendance").select("id, student_id, attendance_date, status").order("attendance_date", { ascending: false }); if (savedTeacherId !== 1 && assignedStudentIds.length > 0) { attendanceQuery = attendanceQuery.in("student_id", assignedStudentIds); } const { data: attendanceData, error: attendanceError } = await attendanceQuery;
